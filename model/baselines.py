    ...
    for comp in SCORED:
        ...
        with warnings.catch_warnings():
            ...
            if engine == "ridge":
                ...
                pred = _bayesian_component_predict(
                    df, use_train_idx, test_idx, Xtr, Xte, y, comp)
...
def _bayesian_component_predict(
    ...
    coefficient prior is enough for a fast closed-form posterior. Rows with high
    posterior predictive std are pulled back toward the training-only position
    prior. This is used as a shrinkage specialist, not as the deployable model.
    """
    te = df.iloc[test_idx]
    by_pos, glob = _position_rate_prior(df, train_idx, comp)
