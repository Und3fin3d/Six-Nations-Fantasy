...
Stores (read-only):
  data/model_player_match.csv  — 2,760 rows x 111 cols (91 features + id/meta/flags)
...
def feature_view(df: pd.DataFrame, mode: str) -> list[str]:
    ...
    cols = list(FEATURE_COLS) + list(FLAG_COLS)
...
assert len(FEATURE_COLS) == 91, f"expected 91 feature cols, got {len(FEATURE_COLS)}"
...
def _selfcheck() -> None:
    ...
    for c in FEATURE_COLS:
        fam = ("FIXTURE" if c.startswith(("opp_", "h2h_", "team_wr", "wr_"))
