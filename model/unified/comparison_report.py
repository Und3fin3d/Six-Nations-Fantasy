...
def report(output: Path, previous: Path, destination: Path, candidate: str):
    ...
    text = "# Fixed-seed candidate: complete four-evaluation comparison\n\n"
    text += "One prespecified candidate; seeds 17, 29 and 43 receive equal weight. No post-result tuning. Lower MAE and higher team points are better.\n\n## Fantasy comparisons\n\n" + _table(summary)
    text += "\n\n## Friendly-15\n\n" + _table(friendly)
    ...
    text += "\n## Limits\n\nSix Nations squads remain price-free diagnostics, not budget-feasible proof. NCR GW3 retains corrected retrospective lineup/prices; GW1 excludes New Zealand and France. Friendly and Six Nations lineups are oracle inputs and kickoff is a lock proxy. Unversioned historical data cannot prove original publication times. All models use the same earlier completed history; no current-game labels enter features. Existing event weights and these historical outcomes have already been examined. NCR GW4-7 is unused. Neither an improved point estimate nor a three-seed average establishes universal superiority. No production switch or automatic PR is authorized.\n"
    text += "\n## Reproduction\n\nUse the pinned Python 3.11 model environment and required LFS inputs. For each fixed fixture run `python -m model.unified.domain_experiment --study seedbag --output results --job test:<fixture_id>`; for fantasy use each of the thirteen existing slate names. Use a fresh output directory. `python -m model.unified.comparison_report --output results --previous previous_results --destination report --candidate p3_seedbag_native` requires the previous evidence for control checks.\n"
